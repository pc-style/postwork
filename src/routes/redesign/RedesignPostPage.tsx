    ...
    >
      <article className="min-w-0 xl:pr-8">
        ...
        <header className="group/post">
          ...
          {!editing ? <PostModeration post={post} onStartEdit={() => setEditing(true)} /> : null}
        </header>
        ...
        {!editing ? (
          <div className="mt-7 max-w-[65ch]">
            ...
            <AttachmentGallery
              attachments={attachments.filter((attachment) => !attachment.replyId)}
            />
          </div>
        ) : null}
        ...
        <section aria-labelledby="replies-heading" className="mt-10">
          ...
          {(repliesResult.status === "CanLoadMore" || repliesResult.status === "LoadingMore") &&
          repliesResult.loadMore ? (
            <Button
              variant="secondary"
              size="sm"
              className="mt-3"
...
function PostEditForm({ post, onDone }: { post: EnrichedPost; onDone: () => void }) {
  const store = useStore();
  ...
  const [submitState, submitAction, isSubmitting] = useActionState(
    async (_previous: { error: string | null }, formData: FormData) => {
      ...
      if (!nextTitle || !nextBody) return { error: null };
      try {
        await store.editPost({ postId: post._id, title: nextTitle, body: nextBody });
        onDone();
        return { error: null };
      } catch (caught) {
        return {
          error: caught instanceof Error ? caught.message : "We couldn't save the post. Try again.",
        };
      }
    },
    { error: null },
  ...
  return (
    <form action={submitAction} className="rounded-lg border border-border bg-surface p-4">
      <ComposerShell
        title={title}
        setTitle={setTitle}
        titleRef={titleRef}
