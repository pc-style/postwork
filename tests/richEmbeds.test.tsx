...
import { collectRichPreviews, MAX_RICH_PREVIEWS_PER_BODY, RichEmbedItems } from "../src/components/RichEmbedList";
import {
  buildRichPreview,
  extractUrls,
  isTrustedGifUrl,
...
describe("rich embed rendering", () => {
  test("renders trusted iframes with security and accessibility constraints", () => {
    const list = RichEmbedItems({ previews: collectRichPreviews("https://youtu.be/dQw4w9WgXcQ") });
  ...
  test("never renders an iframe for an untrusted host", () => {
    const list = RichEmbedItems({ previews: collectRichPreviews("https://example.com/embed/video") });
  ...
  test("caps rendered previews per body", () => {
    const list = RichEmbedItems({
      previews: collectRichPreviews([
        "https://youtu.be/dQw4w9WgXcQ",
