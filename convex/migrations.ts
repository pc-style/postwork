...
export const ensureProductOrg = internalMutation({
  ...
  handler: async (ctx) => {
    const existing = await ctx.db
      .query("orgs")
      .withIndex("by_slug", (q) => q.eq("slug", PRODUCT_ORG_SLUG))
      .unique();
    if (existing) return existing._id;
    return await ctx.db.insert("orgs", {
      name: PRODUCT_ORG_NAME,
      slug: PRODUCT_ORG_SLUG,
      createdAt: Date.now(),
    });
  },
});
...
export const activateFirstProductAdmin = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const admins = await ctx.db
      .query("users")
      .withIndex("by_org_id_and_role", (q) => q.eq("orgId", org._id).eq("role", "admin"))
      .collect();
    if (admins.some((user) => user.status === "active" && !user.deactivatedAt))
  ...
  handler: async (ctx) => {
    ...
    for (const table of [
      ...
      for (const row of await ctx.db.query(table).collect())
        if (!row.orgId) missingOrg.push(`${table}:${row._id}`);
    }
    for (const row of await ctx.db.query("posts").collect()) {
      ...
      if (row.spaceId && (!space || space.orgId !== row.orgId))
        crossOrg.push(`posts:${row._id}:spaceId`);
      for (const participantId of row.participantIds) {
        const participant = await ctx.db.get(participantId);
        if (!participant || participant.orgId !== row.orgId)
          crossOrg.push(`posts:${row._id}:participantIds`);
      }
    }
    for (const row of await ctx.db.query("replies").collect()) {
      ...
      if (row.parentId && (!parent || parent.orgId !== row.orgId || parent.postId !== row.postId))
        crossOrg.push(`replies:${row._id}:parentId`);
    }
    ...
    for (const row of await ctx.db.query("postAttachments").collect()) {
      ...
      if (!uploader || uploader.orgId !== row.orgId)
        crossOrg.push(`postAttachments:${row._id}:uploadedBy`);
      if (row.replyId && (!reply || reply.orgId !== row.orgId || reply.postId !== row.postId))
        crossOrg.push(`postAttachments:${row._id}:replyId`);
    }
    for (const row of await ctx.db.query("agentTasks").collect()) {
      ...
      if (!requester || requester.orgId !== row.orgId)
        crossOrg.push(`agentTasks:${row._id}:requestedById`);
      if (row.connectorId) {
        const connector = await ctx.db.get(row.connectorId);
        if (!connector || connector.orgId !== row.orgId || connector.agentId !== row.agentId)
          crossOrg.push(`agentTasks:${row._id}:connectorId`);
      }
    }
    for (const row of await ctx.db.query("connectors").collect()) {
      ...
      if (!agent?.isAgent || agent.orgId !== row.orgId)
        crossOrg.push(`connectors:${row._id}:agentId`);
      if (!creator || creator.orgId !== row.orgId)
        crossOrg.push(`connectors:${row._id}:createdById`);
    }
    for (const row of await ctx.db.query("connectorEvents").collect()) {
      const connector = await ctx.db.get(row.connectorId);
      if (!connector || connector.orgId !== row.orgId)
        crossOrg.push(`connectorEvents:${row._id}:connectorId`);
    }
    return {
